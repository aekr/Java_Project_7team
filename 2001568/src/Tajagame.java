...
public class Tajagame extends JFrame {
   ...
   private KeyListener second_Listener;
    public Tajagame()
    {
        ...
        // 배경음악 재생
        BackgroundMusic bgm = new BackgroundMusic();
        bgm.playLoopBackgroundMusic("lol.wav");
        first_start();
    }
    class ImagePanel extends JPanel { // 배경 이미지를 그리는 JPanel 서브클래스
        private Image background = new ImageIcon(Tajagame.class.getResource("ex.jpg")).getImage();
...
//            game = false;
    ...
    private void startGameTimer2() {
        ...
        gameTimer2.schedule(new TimerTask() {
            ...
            gameOverLabel2.setHorizontalAlignment(JLabel.CENTER); // 라벨 내용 중앙 정렬
            
            // keyPanel에 라벨 추가
            keyPanel.removeAll(); // 기존에 있던 컴포넌트 제거
            keyPanel.setLayout(new BorderLayout()); // 레이아웃 매니저를 BorderLayout으로 설정
            keyPanel.add(gameOverLabel2, BorderLayout.CENTER); // 라벨을 패널 중앙에 추가
...
//            second_start();
    ...
    private void checkKey(char key) {
        ...
        if (key == expectedKey) {
            ...
            index++;
            BackgroundMusic bgm = new BackgroundMusic();
            bgm.playOneBackgroundMusic("Pling-Sound.wav");
    ...
    private void displayKeysInPanel(String keys) {
        ...
        gbc.insets = new Insets(10, 10, 10, 10); // 여기서 간격을 설정
